...
class RocketLaunching2(RankModel):
    """RocketLaunching2 model with DCN+Deep booster and MLP light.

    The booster (teacher) uses serial architecture: CrossV2 -> LayerNorm -> Deep MLP.
    The light (student) uses a simple MLP, distilling from booster's deep hidden layers.
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        # ===== Booster branch: CrossV2 -> LN -> Deep MLP (serial) =====
        self.booster_cross = CrossV2(
            input_dim=booster_input_dim, **config_to_kwargs(self._model_config.cross)
        )
        self.booster_cross_ln = nn.LayerNorm(self.booster_cross.output_dim())

        self.booster_deep = MLP(
            in_features=self.booster_cross.output_dim(),
        ...
        self.booster_linear = nn.Linear(
            self.booster_deep.output_dim(), self._num_class, bias=False
        )
    ...
    def _get_distillation_mlp_index(self) -> Dict[int, int]:
        booster_deep_units = self._model_config.deep.hidden_units
    ...
    def predict(self, batch: Batch) -> Dict[str, torch.Tensor]:
        ...
        if self.training:
            # Booster: CrossV2 -> LN -> Deep MLP -> LN -> Linear (serial)
            cross_out = self.booster_cross(share_net)
            cross_out = self.booster_cross_ln(cross_out)

            booster_deep_raw = self.booster_deep(cross_out)
            if self.return_hidden_layer_feature:
                deep_out = booster_deep_raw["hidden_layer_end"]
            else:
                deep_out = booster_deep_raw
            deep_out = self.booster_deep_ln(deep_out)

            booster_out = self.booster_linear(deep_out)
            ...
            for i, j in self.mlp_index_dict.items():
                ...
                prediction_dict[f"booster_{j}"] = booster_deep_raw[
                    "hidden_layer" + str(j)
                ]
    ...
    def feature_based_sim(
        ...
        """Compute similarity between booster_deep and light_net."""
