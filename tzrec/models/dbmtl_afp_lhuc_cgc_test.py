...
"""Tests for tzrec.models.dbmtl_afp_lhuc_cgc."""
...
from tzrec.models.dbmtl_afp_lhuc_cgc import DBMTL_AFP_LHUC_CGC
...
def _make_features():
    """Three id features in a single DEEP group; cat_a/cat_b used as bias."""
    feature_cfgs = [
...
def _base_config(task_towers, **kw):
    ...
    if kw.pop("afp_soft", False):
        afp_block = {
            "afp_soft": multi_task_rank_pb2.AFPSoftConfig(
    ...
    cfg = multi_task_rank_pb2.DBMTL_AFP_LHUC_CGC(
        **afp_block,
        dcnv2=module_pb2.CrossV2(cross_num=2, low_rank=16),
        # EP gate reads bias_feature_names from the group (no AFP).
    ...
    return features, model_pb2.ModelConfig(
        feature_groups=feature_groups, dbmtl_afp_lhuc_cgc=cfg
    )


class DBMTLAFPLHUCCGCTest(unittest.TestCase):
    """DBMTL_AFP_LHUC_CGC model test."""
    ...
    def test_independent(self, graph_type) -> None:
        """Forward passes; EP gate uses bias_feature_names, PP net uses AFP S."""
        task_towers = [
            _bayes_tower("is_click", "label1"),
            _bayes_tower("is_buy", "label2"),
        ]
        features, model_config = _base_config(task_towers)
        model = DBMTL_AFP_LHUC_CGC(
            model_config, features, labels=["label1", "label2"]
        )
        # EP gate reads bias features cat_a/cat_b from the group.
        self.assertEqual(set(model._bias_feature_dims), {"cat_a", "cat_b"})
    ...
    def test_without_pp_net_and_without_bottom_mlp(self) -> None:
        """DCNv2-only branch, AFP O stream feeds CGC directly when no PP net."""
        task_towers = [
            _bayes_tower("is_click", "label1"),
            _bayes_tower("is_buy", "label2"),
        ]
        features, model_config = _base_config(task_towers)
        model_config.dbmtl_afp_lhuc_cgc.ClearField("lhuc_pp_net")
        model = DBMTL_AFP_LHUC_CGC(
            model_config, features, labels=["label1", "label2"]
        )
        init_parameters(model, device=torch.device("cpu"))
    ...
    def test_afp_ste_gradient_all_nonzero(self) -> None:
        """AFP mask weights receive non-zero gradient via STE even when M==0."""
        task_towers = [
            _bayes_tower("is_click", "label1"),
            _bayes_tower("is_buy", "label2"),
        ]
        ...
        model.train()
        with torch.no_grad():
            model.afp.weight.fill_(-5.0)  # force M == 0 everywhere
        batch = _make_batch()
        predictions = model(batch)
        losses = model.loss(predictions, batch)
        sum(losses.values()).backward()
        self.assertIsNotNone(model.afp.weight.grad)
        self.assertTrue(torch.all(model.afp.weight.grad != 0))

    def test_afp_soft_forward_and_loss(self) -> None:
        """afp_soft oneof builds AFPModule2; entropy reg flows into losses."""
        ...
        features, model_config = _base_config(task_towers, afp_soft=True)
        model = DBMTL_AFP_LHUC_CGC(
            model_config, features, labels=["label1", "label2"]
        )
        init_parameters(model, device=torch.device("cpu"))
        self.assertIsInstance(model.afp, AFPModule2)
        model.train()
        batch = _make_batch()
        predictions = model(batch)
        self.assertEqual(predictions["probs_is_click"].size(), (2,))
        self.assertEqual(predictions["probs_is_buy"].size(), (2,))
        losses = model.loss(predictions, batch)
        # entropy regulariser must be registered as an aux loss.
        self.assertIn("afp_entropy_p_loss", losses)
        sum(losses.values()).backward()
        # gate_mlp params received gradient via the entropy reg path.
        for param in model.afp.gate_body.parameters():
            self.assertIsNotNone(param.grad)
        for param in model.afp.gate_head.parameters():
            self.assertIsNotNone(param.grad)

    def test_invalid_bias_feature_rejected(self) -> None:
        """bias_feature_names not in the group must be rejected."""
        task_towers = [
            _bayes_tower("is_click", "label1"),
            _bayes_tower("is_buy", "label2"),
        ]
        features, model_config = _base_config(task_towers)
        model_config.dbmtl_afp_lhuc_cgc.lhuc_gate.bias_feature_names.append(
            "nonexistent"
        )
    ...
    def test_invalid_model_type_rejected(self) -> None:
        """A non-dbmtl_afp_lhuc_cgc config must be rejected."""
        task_towers = [
            _bayes_tower("is_click", "label1"),
            _bayes_tower("is_buy", "label2"),
        ]
        ...
        model_config.ClearField("dbmtl_afp_lhuc_cgc")
