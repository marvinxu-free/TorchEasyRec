...
from tzrec.models.dcn_v3 import DCNV3
...
class DCNV3Test(unittest.TestCase):
    @parameterized.expand(
        [[TestGraphType.NORMAL], [TestGraphType.FX_TRACE], [TestGraphType.JIT_SCRIPT]]
    )
    def test_dcn_v3_with_bias(self, graph_type) -> None:
        """Test DCNV3 with bias feature group."""
        feature_cfgs = [
        ...
        dcn_v3 = DCNV3(model_config=model_config, features=features, labels=["label"])
        init_parameters(dcn_v3, device=torch.device("cpu"))
        dcn_v3 = create_test_model(dcn_v3, graph_type)
        ...
        if graph_type == TestGraphType.JIT_SCRIPT:
            predictions = dcn_v3(batch.to_dict())
        else:
            predictions = dcn_v3(batch)
    ...
    def test_dcn_v3_without_bias(self, graph_type) -> None:
        """Test DCNV3 without bias feature group (backward compatible)."""
        feature_cfgs = [
            feature_pb2.FeatureConfig(
                id_feature=feature_pb2.IdFeature(
                    feature_name="cat_a", embedding_dim=16, num_buckets=100
