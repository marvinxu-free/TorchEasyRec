...
Implements APPNet (WSDM'26, docs/APPNet.md). The embedding is split by the
Automatic Feature Partitioning (AFP) module into two complementary streams:

    * ``S`` feeds each tower's PPNet, producing per-layer gates g^(l).
    * ``O`` feeds each tower's DCNv2 + gated DNN.

Each task tower independently runs DCNv2(O) -> Linear-ReLU-*g^(l) stack. Task
dependency mirrors MTL_LHUC:

    * no dependency: towers independent
    * DBMTL: a tower sets relation_tower_names + relation_mlp (concat dependency)
    * ESMM: model-level esmm { ctr_tower_name, cvr_tower_name }; the CVR tower
      is supervised only through CTCVR = sigmoid(CTR) * sigmoid(CVR).

The loss / ESMM / bias-task / sample_weight_fusion / pcgrad paths are shared
with MTL_LHUC; only the predict() backbone differs.
"""
...
from tzrec.modules.afp import AFPModule, APPNetTower
...
class MTL_APPNet(MultiTaskRank):
    """MTL_APPNet model: AFP + per-tower DCNv2 + PPNet-gated DNN.
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        assert (
            model_config.WhichOneof("model") == "mtl_appnet"
        ), "invalid model config: %s" % self._model_config.WhichOneof("model")
        assert isinstance(self._model_config, multi_task_rank_pb2.MTL_APPNet)
        ...
        # AFP module (required) -- automatic feature partitioning.
        ...
        # Per-tower DCNv2 + PPNet-gated DNN towers
        ...
        for task_tower_cfg in self._task_tower_cfgs:
            ...
            if task_tower_cfg.HasField("pp_net"):
                pp_cfg = task_tower_cfg.pp_net
                gate_hidden_units = list(pp_cfg.gate_hidden_units)
                gamma = pp_cfg.gamma
                gate_activation = pp_cfg.activation or "nn.ReLU"
            else:
                gate_hidden_units = None
                gamma = 2.0
                gate_activation = "nn.ReLU"
            dnn_hidden_units = list(task_tower_cfg.dnn_hidden_units)
            assert len(dnn_hidden_units) > 0, (
                f"tower '{tower_name}' must set dnn_hidden_units"
            )
            dropout = (
                task_tower_cfg.dnn_dropout_ratio
                if task_tower_cfg.dnn_dropout_ratio > 0
                else None
            )
            tower = APPNetTower(
                ...
                use_ln=task_tower_cfg.dnn_use_ln,
    ...
    def predict(self, batch: Batch) -> Dict[str, torch.Tensor]:
        ...
        # AFP: partition embedding into PPNet stream S and DNN stream O.
        S, O = self.afp(net)

        # Per-tower DCNv2(O) + PPNet-gated DNN.
        task_net: Dict[str, torch.Tensor] = {}
        for task_tower_cfg in self._task_tower_cfgs:
            tower_name = task_tower_cfg.tower_name
            task_net[tower_name] = self.towers[tower_name](O, S)
