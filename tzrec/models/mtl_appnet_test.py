...
"""Tests for tzrec.models.mtl_appnet."""
...
def _base_appnet_config(task_towers, afp_mode=multi_task_rank_pb2.AFPConfig.FEATURE_WISE, **kw):
    features, feature_groups = _make_features()
    cfg = multi_task_rank_pb2.MTL_APPNet(
    ...
    return features, model_pb2.ModelConfig(
        feature_groups=feature_groups, mtl_appnet=cfg
    )


def _appnet_tower(name, label, relation=False):
    tower = multi_task_rank_pb2.APPNetTower(
        tower_name=name,
        label_name=label,
        dcnv2=module_pb2.CrossV2(cross_num=2, low_rank=8),
        dnn_hidden_units=[16, 8],
        dnn_use_ln=True,
        pp_net=multi_task_rank_pb2.APPNetPPNetConfig(
            gate_hidden_units=[8], gamma=2.0
        ),
        losses=[
            loss_pb2.LossConfig(
...
class MTLAPPNetTest(unittest.TestCase):
    ...
    def test_independent(self, afp_mode) -> None:
        task_towers = [
            _appnet_tower("is_click", "label1"),
            _appnet_tower("is_buy", "label2"),
        ]
        features, model_config = _base_appnet_config(task_towers, afp_mode=afp_mode)
        model = MTL_APPNet(model_config, features, labels=["label1", "label2"])
    ...
    def test_dbmtl_dependency(self) -> None:
        task_towers = [
            _appnet_tower("is_click", "label1"),
            _appnet_tower("is_buy", "label2", relation=True),
        ]
        features, model_config = _base_appnet_config(task_towers)
        model = MTL_APPNet(model_config, features, labels=["label1", "label2"])
        init_parameters(model, device=torch.device("cpu"))
        batch = _make_batch()
        predictions = model(batch)
    ...
    def test_esmm_dependency(self) -> None:
        ...
        features, model_config = _base_appnet_config(
            task_towers,
    ...
    def test_gradient_decoupling(self) -> None:
        # Each feature field should receive gradient from exactly one of the
        # two streams (S / O). With LN identity (single field, all mask=1),
        # the DNN-stream (O) carries gradient and the PPNet-stream (S) side is
        # stop-gradient for that field; verify AFP weight grad is finite and
        # embedding grad is non-zero.
        task_towers = [_appnet_tower("is_click", "label1")]
        features, model_config = _base_appnet_config(task_towers)
        model = MTL_APPNet(model_config, features, labels=["label1"])
        init_parameters(model, device=torch.device("cpu"))
        model.train()
        batch = _make_batch()
        predictions = model(batch)
        ...
        self.assertTrue(torch.isfinite(model.afp.weight.grad).all())
