...
from tzrec.models.rocket_launching2 import RocketLaunching2
...
class RocketLaunching2Test(unittest.TestCase):
    ...
    def test_rocket_launching2(self, graph_type, is_training=True) -> None:
        feature_cfgs = [
        ...
        model_config = model_pb2.ModelConfig(
            ...
            rocket_launching2=general_rank_model_pb2.RocketLaunching2(
                share_mlp=module_pb2.MLP(hidden_units=[128, 64]),
                cross=module_pb2.CrossV2(cross_num=2, low_rank=32),
                deep=module_pb2.MLP(hidden_units=[32, 16, 8]),
                light_mlp=module_pb2.MLP(hidden_units=[16, 8]),
                feature_based_distillation=True,
            ),
        )
        rocket_launching2 = RocketLaunching2(
            model_config=model_config,
            features=features,
            labels=["label"],
        )
        init_parameters(rocket_launching2, device=torch.device("cpu"))
        if not is_training:
            rocket_launching2.eval()
        rocket_launching2 = create_test_model(rocket_launching2, graph_type)
        ...
        if graph_type == TestGraphType.JIT_SCRIPT:
            predictions = rocket_launching2(batch.to_dict())
        else:
            predictions = rocket_launching2(batch)
