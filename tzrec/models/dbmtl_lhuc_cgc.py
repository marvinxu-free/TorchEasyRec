...
"""DBMTL_LHUC_CGC: (dcnv2 || dnn) -> AFP -> LHUC EP gate -> AFP -> LHUC PPNet -> CGC -> DBMTL towers.
...
Architecture flow::
    ...
    Embedding (group_name)
        ...
        -> AFP1(rep) -> S1, O1                      # bit_wise partition
        -> rep = O1 * LHUCEPGate(S1)                # EP personalization (gate_input = S1)
        -> AFP2(rep) -> S2, O2                      # bit_wise partition
        -> rep = LHUCPPNet(O2, gate_input=S2)       # gated DNN (optional but typical)
...
Compared to ``DBMTL_LHUC`` this (a) replaces MMoE with stacked PLE/CGC
``ExtractionNet``s, and (b) inserts two AFP (bit_wise) stages whose ``S`` streams
feed the LHUC EP gate and LHUC PP net respectively, replacing
``DBMTL_LHUC``'s manual ``bias_feature_names``. The AFP STE dual-stream
decouples the personalization-gate gradient from the main DNN path.
"""
...
def _afp_mode(afp_cfg: multi_task_rank_pb2.AFPConfig) -> str:
    """Map the AFPConfig mode enum to the AFPModule mode string."""
    if afp_cfg.mode == multi_task_rank_pb2.AFPConfig.FEATURE_WISE:
        return "feature_wise"
    return "bit_wise"


class DBMTL_LHUC_CGC(MultiTaskRank):
    """DBMTL_LHUC_CGC model.
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        assert (
            model_config.WhichOneof("model") == "dbmtl_lhuc_cgc"
        ), "invalid model config: %s" % self._model_config.WhichOneof("model")
        assert isinstance(self._model_config, multi_task_rank_pb2.DBMTL_LHUC_CGC)
        ...
        # AFP1 (before the EP gate). bit_wise only needs total_dim, so we pass a
        # single pseudo-field of width concat_dim; the per-field structure is no
        # longer meaningful on this learned representation.
        afp1_cfg = self._model_config.afp1
        self.afp1 = AFPModule(
            [concat_dim],
            mode=_afp_mode(afp1_cfg),
            threshold=afp1_cfg.threshold,
            use_ln=afp1_cfg.use_layer_norm,
        )

        # Shared LHUC EP gate; gate_input is afp1's S stream (dim = concat_dim).
        gate_cfg = self._model_config.lhuc_gate
        hidden_units = list(gate_cfg.hidden_units) if gate_cfg.hidden_units else []
        ...
        # AFP2 (before the PP net), runs on the EP-gated representation.
        afp2_cfg = self._model_config.afp2
        self.afp2 = AFPModule(
            [concat_dim],
            mode=_afp_mode(afp2_cfg),
            threshold=afp2_cfg.threshold,
            use_ln=afp2_cfg.use_layer_norm,
        )

        # Optional LHUCPPNet; when present, it widens the CGC input dim.
        # gate_input is afp2's S stream (dim = concat_dim).
        ...
        # CGC stacked ExtractionNets (mirrors MTL_LHUC / MTL_APPNet_LHUC_CGC / PLE).
        assert (
            len(self._model_config.extraction_networks) > 0
        ), "dbmtl_lhuc_cgc requires at least one extraction_network"
    ...
    def predict(self, batch: Batch) -> Dict[str, torch.Tensor]:
        ...
        # AFP1 -> LHUC EP gate (gate_input = S1)
        s1, o1 = self.afp1(rep)
        rep = o1 * self.lhuc_ep_gate(s1)

        # AFP2 -> LHUC PP net (gate_input = S2)
        s2, o2 = self.afp2(rep)
        if self.lhuc_pp_net is not None:
            rep = self.lhuc_pp_net(o2, s2)
        else:
            rep = o2
