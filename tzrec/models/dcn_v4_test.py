...
from tzrec.models.dcn_v4 import DCNV4
...
class DCNV4Test(unittest.TestCase):
    @parameterized.expand(
        [[TestGraphType.NORMAL], [TestGraphType.FX_TRACE], [TestGraphType.JIT_SCRIPT]]
    )
    def test_dcn_v4(self, graph_type) -> None:
        """Test DCNV4 serial architecture."""
        feature_cfgs = [
        ...
        dcn_v4 = DCNV4(model_config=model_config, features=features, labels=["label"])
        init_parameters(dcn_v4, device=torch.device("cpu"))
        dcn_v4 = create_test_model(dcn_v4, graph_type)
        ...
        if graph_type == TestGraphType.JIT_SCRIPT:
            predictions = dcn_v4(batch.to_dict())
        else:
            predictions = dcn_v4(batch)
    ...
    def test_dcn_v4_with_backbone(self, graph_type) -> None:
        """Test DCNV4 with optional backbone."""
        feature_cfgs = [
            feature_pb2.FeatureConfig(
                id_feature=feature_pb2.IdFeature(
                    feature_name="cat_a", embedding_dim=16, num_buckets=100
        ...
        model_config = model_pb2.ModelConfig(
            feature_groups=feature_groups,
            dcn_v4=rank_model_pb2.DCNV4(
                backbone=module_pb2.MLP(hidden_units=[16, 8]),
        ...
        sparse_feature = KeyedJaggedTensor.from_lengths_sync(
            ...
            lengths=torch.tensor([1, 2, 1, 1]),
        )

        batch = Batch(
            dense_features={BASE_DATA_GROUP: KeyedTensor.from_tensor_list(keys=[], tensors=[])},
            sparse_features={BASE_DATA_GROUP: sparse_feature},
            labels={},
        )
        if graph_type == TestGraphType.JIT_SCRIPT:
